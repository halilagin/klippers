...
import { Plan } from './PlanDescriptions'; // Import the Plan interface
...
const SubscriptionPlan: React.FC<PlanProps> = ({ plan, onSelect, selectedPlanId }) => {
  const recommendedColor = '#455cff'; // Define recommended color
  ...
  // Show recommended styling if the plan is recommended AND (no plan is selected OR this plan is the one selected)
  const shouldDisplayAsRecommended = plan.recommended && !isAnotherPlanSelected;

  // Determine border and chip visibility based on calculated states
  const finalBorderColor = isThisPlanSelected ? recommendedColor : (shouldDisplayAsRecommended ? recommendedColor : 'divider');
