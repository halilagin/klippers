...
from .transcribe import transcribe_command
...
def run_all_steps(ctx, input_video, segment_count):
    ...
    audio_file = f"{root_dir}/{os.path.basename(input_video)}.mp3"
    ...
    ctx.invoke(
        transcribe_command, audio_file=audio_file, output_srt_file=srt_file
    )
